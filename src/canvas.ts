...
export default class ZpgraphCanvasRenderer {
  ...
  static _decimateByPixel(iter: utils.Iterator, plotArea: PlotArea) {
    ...
    for (let i = start; i < limit; i++) {
      ...
      last = point;
      if (lowest === null || y < lowest.canvasy!) {
        lowest = point;
      }
      if (highest === null || y > highest.canvasy!) {
        highest = point;
      }
    }
    flush();
  ...
  static _drawSeries(
    ...
    const pointsOnLine: PointOnLine[] = [];
  ...
  static _drawPointsOnLine(
    ...
    const needsIsolation = drawPointCallback !== utils.Circles.DEFAULT;
  ...
  _renderLineChart(
    ...
    for (i = 0; i < plotters.length; i++) {
      ...
      for (let j = 0; j < sets.length; j++) {
        ...
        const strokeWidth = this.zpgraph_.getNumericOption(
          "strokeWidth",
          setName,
        );
  ...
  static _errorPlotter(e: CanvasPlotterEvent) {
    ...
    const iter = utils.createIterator(
      points,
      0,
      points.length,
      ZpgraphCanvasRenderer._getIteratorPredicate(
  ...
  static _fastCanvasProxy(context: CanvasRenderingContext2D): FastCanvasProxy {
    ...
    const compressActions = (opt_losslessOnly?: boolean) => {
    ...
    const flushActions = (opt_noLossyCompression?: boolean) => {
    ...
    const addAction = (action: number, x: number, y: number) => {
