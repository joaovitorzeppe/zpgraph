...
assert.equal(typeof esm.default, 'function', 'default export must be Zgraph');
assert.equal(typeof esm.Zgraph, 'function', 'named Zgraph export missing');
...
assert.equal(
  typeof cjs.default,
  'function',
  'CJS default export must be Zgraph',
);
...
for (const name of ['toRGB_', 'dateString_', 'nonInteractiveModel_']) {
  assert.ok(!(name in esm.default), `Zgraph still exposes ${name}`);
}
