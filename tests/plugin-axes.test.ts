...
const makeChart = (options: ZgraphOptions = {}) => {
  const el = mountDiv();
  const g = new Zgraph(el, sampleData, {
    ...base,
    ...options,
...
describe('Axes plugin', () => {
  ...
  it('creates a label div per tick on both axes', () => {
    ...
    const xLabels = el.querySelectorAll('.zgraph-axis-label-x');
    const yLabels = el.querySelectorAll('.zgraph-axis-label-y');
    expect(xLabels.length).toBeGreaterThan(0);
    expect(yLabels.length).toBeGreaterThan(0);

    // Every tick div carries the shared class as well.
    for (const node of [...xLabels, ...yLabels]) {
      expect(node.classList.contains('zgraph-axis-label')).toBe(true);
  ...
  it('positions x labels below the plot area and y labels to its left', () => {
    ...
    const xOuter = el.querySelector('.zgraph-axis-label-x')!.parentElement!;
    ...
    const yOuter = el.querySelector('.zgraph-axis-label-y')!.parentElement!;
  ...
  it('splits labels between y1 and y2 when a series moves to the second axis', () => {
    ...
    expect(el.querySelectorAll('.zgraph-axis-label-y1').length).toBeGreaterThan(
      0,
    );
    expect(el.querySelectorAll('.zgraph-axis-label-y2').length).toBeGreaterThan(
      0,
    );

    // The right-hand labels sit past the right edge of the plot area.
    const area = g.getArea();
    const y2Outer = el.querySelector('.zgraph-axis-label-y2')!.parentElement!;
  ...
  it('runs tick values through a custom axisLabelFormatter', () => {
    ...
    const xTexts = texts(el, '.zgraph-axis-label-x');
    const yTexts = texts(el, '.zgraph-axis-label-y');
  ...
  it('draws no labels at all when drawAxis is off for every axis', () => {
    ...
    expect(el.querySelectorAll('.zgraph-axis-label')).toHaveLength(0);
  ...
  it('keeps only the x labels when the y axis is turned off', () => {
    ...
    expect(el.querySelectorAll('.zgraph-axis-label-x').length).toBeGreaterThan(
      0,
    );
    expect(el.querySelectorAll('.zgraph-axis-label-y')).toHaveLength(0);
  ...
  it('detaches the old labels on redraw instead of stacking new ones', () => {
    const { el, g } = makeChart();
    const before = el.querySelectorAll('.zgraph-axis-label').length;

    g.updateOptions({ valueRange: [0, 50] });
    g.updateOptions({ valueRange: [0, 50] });

    expect(el.querySelectorAll('.zgraph-axis-label').length).toBeGreaterThan(0);
    expect(
      el.querySelectorAll('.zgraph-axis-label').length,
    ).toBeLessThanOrEqual(before * 2);
    // No duplicate label text for the same tick.
    const ys = texts(el, '.zgraph-axis-label-y');
  ...
  it('reuses the label nodes across redraws instead of recreating them', () => {
    const { el, g } = makeChart();
    const before = Array.from(el.querySelectorAll('.zgraph-axis-label-x'));

    g.updateOptions({ dateWindow: [2, 8] });

    const after = Array.from(el.querySelectorAll('.zgraph-axis-label-x'));
    expect(after.length).toBeGreaterThan(0);
    // Same nodes, new text.
    expect(after[0]!).toBe(before[0]!);
    expect(texts(el, '.zgraph-axis-label-x')).not.toEqual(before.map(() => ''));
  ...
  it('removes the labels of an axis that stops being drawn', () => {
    const { el, g } = makeChart();
    expect(el.querySelectorAll('.zgraph-axis-label-y').length).toBeGreaterThan(
      0,
    );

    g.updateOptions({ axes: { y: { drawAxis: false } } });

    expect(el.querySelectorAll('.zgraph-axis-label-y').length).toBe(0);
    expect(el.querySelectorAll('.zgraph-axis-label-x').length).toBeGreaterThan(
      0,
    );
