...
const xAxisOpts = (g: Zgraph): AxisOptions => {
  if (!g.attrs_.axes) {
    g.attrs_.axes = {};
  }
  if (!g.attrs_.axes.x) {
 ...
 * @private
 */
export const detectTypeFromString = (g: Zgraph, str: string): void => {
  let isDate = false;
 ...
 */
export const parseCSV = (g: Zgraph, data: string): RawData => {
  const ret: RawData = [];
  ...
  for (let i = start; i < lines.length; i++) {
    ...
    if (i === 0 && g.attr_('labels')) {
      ...
      if (all_null) {
        log.warn(
          "The zgraph 'labels' option is set, but the first row " +
            "of CSV data ('" +
            line +
  ...
  if (outOfOrder) {
    ...
    ret.sort(function (a: RawDataRow, b: RawDataRow) {
 ...
 * are numeric, this is the same as zgraph' internal format. If the x values
 ...
 * @private
 */
export const parseDataTable = (g: Zgraph, data: GvizDataTable): void => {
  const shortTextForAnnotationNum = function (num: number): string {
  ...
  for (i = 0; i < rows; i++) {
    ...
    if (!g.getBooleanOption('errorBars')) {
      ...
      // Strip out infinities, which give zgraph problems later on.
