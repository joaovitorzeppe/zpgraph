...
import type { LegendData, ZgraphOptions } from '../src/types';
...
const makeChart = (options: ZgraphOptions = {}) => {
  ...
  const legend = el.querySelector('.zgraph-legend') as HTMLElement;
  return { el, g, legend };
};
...
describe('Legend plugin', () => {
  ...
  it("creates its own div and fills it when legend is 'always'", () => {
    ...
    expect(legend.querySelectorAll('.zgraph-legend-line')).toHaveLength(2);
  ...
  it('leaves a series hidden by visibility out of the legend', () => {
    ...
    expect(legend.querySelectorAll('.zgraph-legend-line')).toHaveLength(1);
  ...
  it('renders into a user-supplied labelsDiv and leaves the graph div alone', () => {
    ...
    expect(el.querySelector('.zgraph-legend')).toBeNull();
